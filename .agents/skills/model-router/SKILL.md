...
Use the repository's `codex-auto` recommendation engine as the source of the scoring policy and model registry. For a task provided in chat, run `codex-auto recommend --json "<task>"`; for a task already stored in a text file, use `--prompt-file`. When working in this repository before installation, use `node bin/codex-auto.js` instead. Never put secret task text into a shell command if a prompt file is available.
